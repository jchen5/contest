...
class BugfixJudgeActionHandler extends JudgeActionHandler {
  
  public function submit_judgment($in, &$out) {
    parent::submit_judgment($in, $out);
    try {
      if ($in['correct']) {
        BugfixScoreboardManager::generateScoreboard($in['contest_id'], $in['division_id']);
      }
      else {
        BugfixScoreboardManager::setIncorrect($in['contest_id'], $in['division_id'], $in['team_id'], $in['problem_id']);
      }
    } catch (Exception $e) {
    ...
    if ($g_curr_contest) {
      ...
      if ($metadata) {
        ...
        if (!(isset($metadata['judge_scoreboard']) && isset($metadata['problems']))) {
          BugfixScoreboardManager::generateScoreboard($contest_id, $division_id);
          $metadata = DBManager::getContestDivisionMetadata($contest_id, $division_id);
          $metadata = json_decode($metadata, true);
        }
        if (isset($metadata['judge_scoreboard']) && isset($metadata['problems'])) {
