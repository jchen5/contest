...
class BugfixProblemConfigMetadata extends ProblemConfigMetadata {
...
?>
  ...
  $("#<?= $downloadID ?>").click(function() {
    window.location.href = "handlefile.php?action=download_bugfix_zip&problem_id=" + problemID + "&division_id=" + divisionID + "&contest_id=" + contestID;
  });
  ...
  <tr>
    ...
    <td>
      ...
      <form id="<?= $uploadFormID ?>" action="handlefile.php" method="post" enctype="multipart/form-data" target="<?= $uploadFrameID ?>">
        ...
        <input type="hidden" name="action" value="upload_bugfix_zip"></input>
        <br />
